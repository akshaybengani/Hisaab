...
Map<int, Product> catalogue([List<Product> products = const <Product>[
  formula1,
  afresh,
...
void main() {
  group('productBalance', () {
    test('sums delivery lines and subtracts a payment that settles them', () {
    ...
    test('returns the full total where nothing has been paid', () {
      expect(
    ...
    test('goes negative where a payment covers more than is owed', () {
      final int balance = LedgerMath.productBalance(
    ...
    test('ignores a cash loan and a repayment', () {
      final int balance = LedgerMath.productBalance(
        <DeliveryWithItems>[sep7Delivery],
        <MoneyEntry>[
          money(date: sep3, amountPaise: 500000, kind: MoneyKind.cashLent),
          money(id: 2, date: sep8, amountPaise: 95000, kind: MoneyKind.repayment),
        ],
      );
    ...
    test('an adjustment reduces the product balance', () {
      final int balance = LedgerMath.productBalance(
        <DeliveryWithItems>[sep7Delivery],
        <MoneyEntry>[
          money(date: sep8, amountPaise: 90000, kind: MoneyKind.paymentReceived),
          money(id: 2, date: sep8, amountPaise: 5000, kind: MoneyKind.adjustment),
        ],
      );
    ...
    test('payments with no deliveries leave a credit', () {
      final int balance = LedgerMath.productBalance(
        const <DeliveryWithItems>[],
        <MoneyEntry>[
          money(date: sep8, amountPaise: 20000, kind: MoneyKind.paymentReceived),
        ],
      );
  ...
  group('cashBalance', () {
    ...
    test('a repayment reduces a loan', () {
      expect(
        LedgerMath.cashBalance(<MoneyEntry>[
          money(date: sep3, amountPaise: 500000, kind: MoneyKind.cashLent),
          money(id: 2, date: sep8, amountPaise: 200000, kind: MoneyKind.repayment),
        ]),
        300000,
      );
    });

    test('a product payment never touches the cash pool', () {
      expect(
        LedgerMath.cashBalance(<MoneyEntry>[
          money(date: sep3, amountPaise: 500000, kind: MoneyKind.cashLent),
          money(id: 2, date: sep8, amountPaise: 95000, kind: MoneyKind.paymentReceived),
        ]),
        500000,
      );
    });

    test('an adjustment and a write off never touch the cash pool', () {
      expect(
        LedgerMath.cashBalance(<MoneyEntry>[
          money(date: sep8, amountPaise: 5000, kind: MoneyKind.adjustment),
          money(id: 2, date: sep9, amountPaise: 95000, kind: MoneyKind.writeOff),
        ]),
        0,
      );
    });
  ...
  group('two pools stay separate', () {
    ...
    test('a payment clears the product due and leaves the loan alone', () {
    ...
    test('a repayment pays down the loan and leaves the product due alone', () {
      final List<MoneyEntry> entries = <MoneyEntry>[
        loan,
        money(id: 2, date: sep8, amountPaise: 95000, kind: MoneyKind.repayment),
      ];
  ...
  group('allocate', () {
    test('a partial payment spans two deliveries oldest first', () {
      final List<DeliveryWithItems> deliveries = <DeliveryWithItems>[
        sep3Delivery,
        sep7Delivery,
      ];
    ...
    test('one payment covering several deliveries marks them all paid', () {
      final List<DeliveryWithItems> deliveries = <DeliveryWithItems>[
        sep3Delivery,
        sep7Delivery,
      ];
      final List<MoneyEntry> entries = <MoneyEntry>[
        money(date: sep8, amountPaise: 505000, kind: MoneyKind.paymentReceived),
      ];
    ...
    test('a payment larger than everything owed still marks each paid', () {
      final List<DeliveryWithItems> deliveries = <DeliveryWithItems>[
        sep3Delivery,
        sep7Delivery,
      ];
      final List<MoneyEntry> entries = <MoneyEntry>[
        money(date: sep8, amountPaise: 600000, kind: MoneyKind.paymentReceived),
      ];
    ...
    test('several payments add up before they are allocated', () {
      final List<DeliveryWithItems> deliveries = <DeliveryWithItems>[
        sep3Delivery,
        sep7Delivery,
      ];
      final List<MoneyEntry> entries = <MoneyEntry>[
        money(date: sep8, amountPaise: 200000, kind: MoneyKind.paymentReceived),
        money(id: 2, date: sep9, amountPaise: 250000, kind: MoneyKind.paymentReceived),
      ];
    ...
    test('order in the input list does not change the outcome', () {
      final List<DeliveryWithItems> newestFirst = <DeliveryWithItems>[
        sep7Delivery,
        sep3Delivery,
      ];
      final List<MoneyEntry> entries = <MoneyEntry>[
        money(date: sep8, amountPaise: 410000, kind: MoneyKind.paymentReceived),
      ];

      expect(LedgerMath.allocate(newestFirst, entries), <int, SettlementState>{
        1: SettlementState.paid,
    ...
    test('nothing paid reads unpaid', () {
      expect(
    ...
    test('a repayment does not settle a delivery', () {
      final List<DeliveryWithItems> deliveries = <DeliveryWithItems>[
        sep7Delivery,
      ];
      final List<MoneyEntry> entries = <MoneyEntry>[
    ...
    test('a write off zeroes the balance without marking a delivery paid', () {
    ...
    test('a refund recorded as outgoing gives back what it took', () {
      final List<DeliveryWithItems> deliveries = <DeliveryWithItems>[
        sep3Delivery,
      ];
      final List<MoneyEntry> entries = <MoneyEntry>[
  ...
  group('coveredPaise', () {
    test('a paid delivery is covered in full', () {
      final List<DeliveryWithItems> deliveries = <DeliveryWithItems>[
        sep3Delivery,
        sep7Delivery,
      ];
  ...
  group('buildStatement', () {
    ...
    test('product lines and cash lines sit in separate groups', () {
      final Statement statement = statementOf(
        deliveries: <DeliveryWithItems>[sep3Delivery, sep7Delivery],
        entries: <MoneyEntry>[
          money(date: sep8, amountPaise: 450000, kind: MoneyKind.paymentReceived),
          money(id: 2, date: sep3, amountPaise: 500000, kind: MoneyKind.cashLent),
          money(id: 3, date: sep9, amountPaise: 100000, kind: MoneyKind.repayment),
        ],
      );
    ...
    test('each subtotal equals the balance function for that pool', () {
      final List<DeliveryWithItems> deliveries = <DeliveryWithItems>[
        sep3Delivery,
        sep7Delivery,
      ];
      final List<MoneyEntry> entries = <MoneyEntry>[
        money(date: sep8, amountPaise: 200000, kind: MoneyKind.paymentReceived),
        money(id: 2, date: sep8, amountPaise: 5000, kind: MoneyKind.adjustment),
        money(id: 3, date: sep3, amountPaise: 60000, kind: MoneyKind.cashBorrowed),
      ];
    ...
    test('lines run oldest first inside each group', () {
      final Statement statement = statementOf(
        ...
        entries: <MoneyEntry>[
          money(date: sep8, amountPaise: 10000, kind: MoneyKind.paymentReceived),
          money(id: 2, date: sep9, amountPaise: 20000, kind: MoneyKind.paymentReceived),
        ],
      );
    ...
    test('a household line surfaces the member it was for', () {
      final Statement statement = statementOf(
        person: const Person(id: 7, name: 'Sharma household', isHousehold: true),
    ...
    test('a round-off is its own line rather than a smaller delivery', () {
      final Statement statement = statementOf(
        deliveries: <DeliveryWithItems>[sep7Delivery],
        entries: <MoneyEntry>[
          money(date: sep8, amountPaise: 90000, kind: MoneyKind.paymentReceived),
          money(id: 2, date: sep9, amountPaise: 5000, kind: MoneyKind.adjustment),
        ],
      );

      expect(statement.products.lines.length, 3);
      expect(statement.products.lines.first.amountPaise, 95000);
      expect(statement.products.lines.last.description, 'Round-off adjustment');
    ...
    test('every money kind reads as itself', () {
      final Statement statement = statementOf(
        ...
        entries: <MoneyEntry>[
          ...
          money(id: 4, date: sep8, amountPaise: 4000, kind: MoneyKind.cashBorrowed),
          money(id: 5, date: sep9, amountPaise: 5000, kind: MoneyKind.repayment),
        ],
      );
    ...
    test('a person with no activity gets two empty groups and a zero net', () {
      final Statement statement = statementOf(
        deliveries: const <DeliveryWithItems>[],
        entries: const <MoneyEntry>[],
      );
    ...
    test('a price change does not rewrite a statement already built', () {
      final List<DeliveryWithItems> deliveries = <DeliveryWithItems>[
        sep3Delivery,
        sep7Delivery,
      ];
      ...
      expect(
        ...
        before.products.lines.map((StatementLine l) => l.amountPaise).toList(),
      );
  ...
  group('balances', () {
    ...
    test('returns one balance per person, sorted by what they owe', () {
      ...
      expect(
        result.map((PersonBalance b) => b.person.name).toList(),
        <String>['Asha', 'Chandra', 'Bina'],
      );
    ...
    test('last activity is the most recent delivery or money entry', () {
      final List<PersonBalance> result = LedgerMath.balances(
        ...
        entriesByPerson: <int, List<MoneyEntry>>{
          7: <MoneyEntry>[
            money(date: sep8, amountPaise: 1000, kind: MoneyKind.paymentReceived),
          ],
        },
      );
