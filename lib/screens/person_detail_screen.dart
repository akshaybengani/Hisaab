...
import '../components/statement_share.dart';
...
class PersonDetailScreen extends StatelessWidget {
  ...
  Future<void> _shareText(AppState state, Statement statement) async {
    final String body = StatementShare.message(
      statement,
      template: state.setting(SettingKeys.templateStatement),
    );
  ...
  Future<void> _sharePdf(AppState state, Statement statement) async {
    final Uint8List bytes = await StatementShare.pdf(
      statement,
      header: state.setting(SettingKeys.pdfHeader),
      footer: state.setting(SettingKeys.pdfFooter),
    );
